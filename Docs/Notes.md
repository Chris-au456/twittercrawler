...
Essentially, since I've done a bit of work with swagger - because of it's presentation and customizability I thought this was 
the best path to take. I set up the skeleton through flask and imported the functionality of obtaining the latest tweets
for a user when a POST request is sent (Interacting with the server itself, as opposed to GET when you're just fetching a resource). 
